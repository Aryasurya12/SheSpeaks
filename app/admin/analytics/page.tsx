...
import { Activity, TrendingUp, TrendingDown, Users, FileText, AlertCircle, RefreshCw } from "lucide-react";
...
export default function AnalyticsPage() {
  ...
  const reportsData = [
    { label: "Harassment", value: getCount("harassment"), trend: "+12%", color: "text-primary", density: Math.round((getCount("harassment") / total) * 100) },
    { label: "Theft", value: getCount("theft"), trend: "-5%", color: "text-emerald-500", density: Math.round((getCount("theft") / total) * 100) },
    { label: "Assault", value: getCount("assault"), trend: "-22%", color: "text-red-500", density: Math.round((getCount("assault") / total) * 100) },
    { label: "Other", value: reports.length - (getCount("harassment") + getCount("theft") + getCount("assault")), trend: "+3%", color: "text-orange-500", density: Math.round(((reports.length - (getCount("harassment") + getCount("theft") + getCount("assault"))) / total) * 100) },
  ];
    ...
    <DashboardLayout role="admin" userEmail="admin@shespeaks.com">
       <div className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-center sm:text-left">
            {reportsData.map((data, idx) => (
              <div key={idx} className="p-10 rounded-[3rem] glass border border-white/5 hover:border-primary/20 transition-all flex flex-col items-center group shadow-2xl relative">
                 ...
                 <div className={cn("px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-white/5 border border-white/10 flex items-center gap-3 shadow-lg", data.trend.startsWith('+') ? 'text-primary' : 'text-emerald-500')}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                   ...
                   {reportsData.map((data, i) => (
                     <div key={i} className="space-y-4">
                        <div className="flex items-center justify-between text-xs font-black uppercase tracking-widest text-foreground/40">
                           ...
                           <span className={cn("font-black", data.color)}>{loading ? "0" : data.density}% Density</span>
                        </div>
                        <div className="w-full h-3 bg-white/5 rounded-full overflow-hidden p-0.5">
                           <div 
                             className={cn("h-full rounded-full transition-all duration-1000 delay-300 bg-primary shadow-[0_0_20px_rgba(147,51,234,0.4)]")} 
                             style={{ width: loading ? '0%' : `${data.density}%` }}
                           />
                        </div>
                     </div>
                   ))}
             ...
             <div className="p-10 rounded-[3.5rem] glass border border-white/5 shadow-2xl flex flex-col items-center justify-center text-center space-y-10 group relative border-2 border-primary/5">
                <div className="w-24 h-24 rounded-full bg-primary/10 flex items-center justify-center p-6 border border-primary/20 shadow-2xl group-hover:scale-110 transition-transform ring-4 ring-primary/5">
                   <AlertCircle className="w-12 h-12 text-primary" />
                </div>
                <div>
                   <h3 className="text-3xl font-black uppercase tracking-tighter mb-4 italic">Strategic <span className="text-primary italic">Heatmap</span></h3>
                   <p className="text-foreground/40 text-sm font-bold uppercase tracking-widest leading-relaxed max-w-sm italic">Detailed safe-zone analytics and route optimization metrics will be automatically generated as platform data scales proportionally.</p>
                </div>
                <button 
                  onClick={fetchData}
